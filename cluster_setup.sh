...
k3d image import hatoo/oha:latest -c demo-cluster-orders-topo
k3d cluster list
...
linkerd viz install --set linkerdVersion=stable-2.14.10 --context topo | kubectl apply -f - --context topo
...
kubectl apply -f linkerd-data-plane-viz-config.yaml --context=topo
